...
class MyDownsample(nn.Module):
    def __init__(self, dim=96, out_dim=192):
        ...
        self.blocks = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Conv2d(out_dim, out_dim, kernel_size=4, stride=2, padding=1, groups=out_dim),
            nn.BatchNorm2d(out_dim),
            nn.SiLU(),
    ...
    def forward(self, x):
        x1 = self.pool(x)
        x2 = self.blocks(x)
        x = self.conv1x1(torch.cat([x1, x2], dim=1))
        return x
...
class VSSM(nn.Module):
    ...
    @staticmethod
    def _make_patch_embed_v8(in_chans=3, embed_dim=96, patch_size=4, patch_norm=True, norm_layer=nn.LayerNorm):
        return nn.Sequential(
            nn.Conv2d(in_chans, embed_dim // 2, kernel_size=4, stride=2, padding=1, bias=True),
            nn.BatchNorm2d(embed_dim // 2),
            nn.SiLU(),
            MyDownsample(embed_dim // 2, embed_dim),
            Permute(0, 2, 3, 1),
            (norm_layer(embed_dim) if patch_norm else nn.Identity()), 
        )
    ...
    @staticmethod
    def _make_downsample_v8(dim=96, out_dim=192, norm_layer=nn.LayerNorm):
        return nn.Sequential(
            Permute(0, 3, 1, 2),
            MyDownsample(dim, out_dim),
            Permute(0, 2, 3, 1),
            norm_layer(out_dim),
        )
